...
if(FALSE){
tag.dimSqueezer <- "DimSqueezer"
DimSqueezer <- function(m){
#   tag <- "DimSqueezer"  ## ?? 
    .Call("DimSqueezer", m,
          get("tag2.dimSqueezer", parent.frame()), "SOD")
}

DimSqueezer_CL<- function(m){
    .Call("DimSqueezer_CL", m, "SOD")
}


squeeze <- function(ds, target.dim, iter.no){
#    if(identical(ds, new('externalptr')))
#        stop("Null DimSqueezer pointer")
    ## the pointer should be checked by the C / C++ code
    ## not in R
    .Call("squeeze", ds, target.dim, iter.no, "SOD")
}
}

## the above seems to be difficult to make safe as I can't seem to
## check the pointer type at the C-level. For whatver, reason, the
## tag disappears. Not sure why, but.. what the hell.
...
DimSqueezer$methods(initialize =
                    ...
                    function(target_dim, iter_no){
                        .Call("squeeze", pointer, target_dim, iter_no)
                    }
                    )
